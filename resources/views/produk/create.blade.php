            ...
            <form action="{{ route('produk.store') }}" method="POST" enctype="multipart/form-data">
                ...
                <div class="form-group">
                    ...
                    <select name="kategori" id="kategori" class="form-control @error('kategori') is-invalid @enderror"
                        ...
                        <option value="SPA" {{ old('kategori') == 'SPA' ? 'selected' : '' }}>Bilateral (SPA)</option>
                        <option value="JFX" {{ old('kategori') == 'JFX' ? 'selected' : '' }}>Multilateral (JFX)</option>
                    </select>
                    @error('kategori')
                        <span class="invalid-feedback">{{ $message }}</span>
                    @enderror
                ...
                <div class="form-group">
                    ...
                    <textarea name="specs" id="specs" rows="5"
                        class="form-control tinymce-editor @error('specs') is-invalid @enderror">{{ old('specs') }}</textarea>
                    @error('specs')
                        <span class="invalid-feedback">{{ $message }}</span>
                    @enderror
                </div>
